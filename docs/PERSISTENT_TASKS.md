...
Admin controla publicación y seguimiento de PR. Dev y Subadmin con el proyecto asignado pueden continuar, pausar o cancelar sus ejecuciones autorizadas; una tarea en solo lectura requiere habilitar escritura explícitamente. Los miembros reciben un resumen del estado: rutas de repositorio, configuración remota y diagnósticos completos quedan reservados al propietario. Los comentarios externos son datos: no se interpretan como selección de MCP, skills, roles ni permisos. Las asignaciones permiten comandos de compilación y pruebas bajo la cuenta del servicio, sin conceder escritura SQL ni MCP externos. El worktree no es un sandbox. Consulta [Equipo](TEAM_ACCESS.md) y [Mi cuenta](USER_ACCOUNTS.md) para permisos y atribución.
