...
Cover:
  - git_flow.diff_file_list: numstat por archivo, badge de pendiente,
    untracked aparte, rename detectado, modos all/committed/pending.
  - git_flow.diff_file: diff de UN archivo, untracked sintetizado,
...
class TestParsersZ(unittest.TestCase):
    def test_numstat_normal_y_rename(self) -> None:
        raw = ("3\t1\tsrc/a.py\0"
               ...
               "-\t-\timg/logo.png\0")
        out = git_flow._parse_numstat_z(raw)
    ...
    def test_name_status_normal_y_rename(self) -> None:
        ...
        out = git_flow._parse_name_status_z(raw)
    ...
    def test_paths_con_acentos_no_se_citan(self) -> None:
        ...
        out = git_flow._parse_numstat_z("1\t0\tsrc/año.py\0")
...
class TestRelInside(unittest.TestCase):
    def test_rechaza_lo_que_sale_del_repo(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            self.assertEqual(git_flow._rel_inside(tmp, "src/a.py"), "src/a.py")
            self.assertEqual(git_flow._rel_inside(tmp, "src\\a.py"), "src/a.py")
            for malo in ("../../.ssh/id_rsa", "/etc/passwd", "C:/Windows/x",
                         "", "   "):
                self.assertIsNone(git_flow._rel_inside(tmp, malo), malo)
...
class TestDiffPorArchivo(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_lista_all_junta_commiteado_y_pendiente(self) -> None:
        out = await git_flow.diff_file_list(str(self.repo), self.branch)
    ...
    async def test_rename_necesita_los_dos_paths(self) -> None:
        ...
        _git(self.repo, "mv", "viejo.py", "renombrado.py")
        _git(self.repo, "commit", "-m", "rename")
        solo = await git_flow.diff_file(str(self.repo), self.branch,
                                        "renombrado.py")
        self.assertIn("new file", solo["diff"])
        con_origen = await git_flow.diff_file(str(self.repo), self.branch,
                                              "renombrado.py",
                                              old_path="viejo.py")
    ...
    async def test_rename_con_old_path_fuera_del_repo_se_ignora(self) -> None:
        ...
        out = await git_flow.diff_file(str(self.repo), self.branch,
                                       "README.md", old_path="../../secreto")
        self.assertIsNone(out["error"])

    async def test_diff_de_un_archivo_solo(self) -> None:
        out = await git_flow.diff_file(str(self.repo), self.branch, "README.md")
        self.assertIsNone(out["error"])
    ...
    async def test_untracked_se_sintetiza_como_altas(self) -> None:
        out = await git_flow.diff_file(str(self.repo), self.branch, "nuevo.txt")
        self.assertIsNone(out["error"])
        ...
        rc, staged = await git_flow._git(str(self.repo), "diff", "--cached",
                                        "--name-only")
    ...
    async def test_binario_se_marca_y_no_manda_bytes(self) -> None:
        ...
        out = await git_flow.diff_file(str(self.repo), self.branch, "blob.bin")
    ...
    async def test_los_warnings_de_git_no_envenenan_el_parseo(self) -> None:
        ...
        uno = await git_flow.diff_file(str(self.repo), self.branch, "crlf.txt")
...
class TestAccionesGit(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_commit_de_paths_selectivo(self) -> None:
        ...
        out = await git_flow.commit_paths(str(self.repo), self.branch,
                                          message="solo uno",
                                          paths=["uno.txt"])
        self.assertIsNone(out["error"])
        ...
        rc, uns = await git_flow._git(str(self.repo), "ls-files", "--others",
                                     "--exclude-standard")
        self.assertEqual(uns.strip(), "dos.txt")

    async def test_commit_sin_mensaje_o_sin_nada_staged(self) -> None:
        sin_msg = await git_flow.commit_paths(str(self.repo), self.branch,
                                               message="   ")
        self.assertIn("mensaje", sin_msg["error"])
        vacio = await git_flow.commit_paths(str(self.repo), self.branch,
                                             message="nada")
    ...
    async def test_commit_con_head_en_otra_rama_no_toca_nada(self) -> None:
        ...
        rc, head = await git_flow._git(str(self.repo), "rev-parse",
                                      "--abbrev-ref", "HEAD")
    ...
    async def test_push_rechaza_ramas_protegidas(self) -> None:
        for protegida in ("main", "master", "develop"):
            out = await git_flow.push_branch(str(self.repo), protegida)
    ...
    async def test_restore_descarta_incluso_lo_staged(self) -> None:
        ...
        out = await git_flow.restore_paths(str(self.repo), self.branch,
                                           ["viejo.py"])
        self.assertIsNone(out["error"])
    ...
    async def test_restore_valida_paths_y_rama(self) -> None:
        fuera = await git_flow.restore_paths(str(self.repo), self.branch,
                                             ["../x"])
        self.assertIn("fuera del repo", fuera["error"])
        vacio = await git_flow.restore_paths(str(self.repo), self.branch, [])
        self.assertIn("al menos un path", vacio["error"])
        protegida = await git_flow.restore_paths(str(self.repo), "main",
                                                  ["README.md"])
    ...
    async def test_merge_sin_pr_no_explota(self) -> None:
        ...
        out = await git_flow.merge_pr(str(self.repo), self.branch)
        self.assertFalse(out["merged"])
        self.assertIsNotNone(out["error"])

    async def test_merge_valida_metodo_y_rama(self) -> None:
        malo = await git_flow.merge_pr(str(self.repo), self.branch,
                                        method="force-push")
        self.assertIn("inválido", malo["error"])
        protegida = await git_flow.merge_pr(str(self.repo), "main")
        self.assertIn("protegida", protegida["error"])

    async def test_pr_sin_commits_no_llama_a_gh(self) -> None:
        out = await git_flow.open_pr(str(self.repo), self.branch,
                                     title="vacío")
...
class TestVisorHttp(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_view_files_y_path(self) -> None:
        ...
        async with TestClient(TestServer(app)) as client:
            ...
            (self.repo / "tocado.txt").write_text("hola\n", encoding="utf-8")
            _git(self.repo, "add", "-A")
