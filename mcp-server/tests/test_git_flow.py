...
Cover:
  ...
  - server: POST /conversations crea rama `<autor>-<fecha>` desde base;
    rechaza (409) una segunda conversación abierta sobre el mismo repo;
    /close abre PR best-effort (gh ausente en CI → pr_error, sin romper).
...
class TestBranchName(unittest.TestCase):
    def test_sanitizes_author(self) -> None:
        ...
        self.assertEqual(git_flow.branch_name("Example User#123"),
                         f"example-user-123-{today}")
        self.assertEqual(git_flow.branch_name(None), f"user-{today}")
        self.assertEqual(git_flow.branch_name("   "), f"user-{today}")


class TestExecCleanup(unittest.IsolatedAsyncioTestCase):
    async def test_spawn_failure_is_an_operational_result(self) -> None:
        with patch("relay.git_flow.asyncio.create_subprocess_exec",
                   AsyncMock(side_effect=FileNotFoundError("gh"))):
            rc, out = await git_flow._exec(".", "gh", "pr")
    ...
    async def test_timeout_reaps_the_process(self) -> None:
        ...
        with patch("relay.git_flow.asyncio.create_subprocess_exec",
            ...
            rc, _ = await git_flow._exec(".", "gh", "pr", timeout=0.01)
    ...
    async def test_cancellation_reaps_the_process(self) -> None:
        ...
        with patch("relay.git_flow.asyncio.create_subprocess_exec",
                   AsyncMock(return_value=proc)):
            task = asyncio.create_task(git_flow._exec(".", "gh", "pr"))
...
class TestOpenBranch(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_creates_and_checks_out_branch(self) -> None:
        self.assertTrue(await git_flow.is_git_repo(str(self.repo)))
        self.assertEqual(await git_flow.detect_base_branch(str(self.repo)), "main")
        branch = await git_flow.open_conversation_branch(str(self.repo), "usuario-demo-a")
        self.assertEqual(branch, git_flow.branch_name("usuario-demo-a"))
        rc, out = await git_flow._git(str(self.repo), "rev-parse", "--abbrev-ref", "HEAD")
    ...
    async def test_base_es_develop_cuando_existe(self) -> None:
        ...
        self.assertEqual(await git_flow.work_base_branch(repo), "develop")
        # detect_base_branch NO cambia: sigue siendo el trunk (lo usa
        # _ensure_develop y el PR).
        self.assertEqual(await git_flow.detect_base_branch(repo), "main")

        branch = await git_flow.open_conversation_branch(repo, "usuario-demo-a")
        # La rama arranca desde develop: el commit de develop está acá.
        self.assertTrue((self.repo / "nuevo.txt").exists())
        rc, out = await git_flow._git(
    ...
    async def test_cada_nuevo_estrena_rama(self) -> None:
        ...
        repo = str(self.repo)
        first = await git_flow.open_conversation_branch(repo, "usuario-demo-a")
        self.assertEqual(first, git_flow.branch_name("usuario-demo-a"))
        _git(self.repo, "checkout", "main")
        second = await git_flow.open_conversation_branch(repo, "usuario-demo-a")
    ...
    async def test_respeta_la_rama_remota_del_pr_anterior(self) -> None:
        ...
        repo = str(self.repo)
        name = git_flow.branch_name("usuario-demo-a")
        rc, sha = await git_flow._git(repo, "rev-parse", "HEAD")
    ...
    async def test_base_cae_al_trunk_sin_develop(self) -> None:
        self.assertEqual(await git_flow.work_base_branch(str(self.repo)), "main")
    ...
    async def test_tracked_modificado_bloquea(self) -> None:
        ...
        with self.assertRaises(git_flow.GitFlowError):
            await git_flow.open_conversation_branch(str(self.repo), "usuario-demo-a")

    async def test_has_commits(self) -> None:
        self.assertTrue(await git_flow.has_commits(str(self.repo)))
        with tempfile.TemporaryDirectory() as t:
            _git(Path(t), "init", "-b", "main")  # sin commit → HEAD unborn
            self.assertFalse(await git_flow.has_commits(t))
...
class TestSyncBaseWithOrigin(unittest.IsolatedAsyncioTestCase):
    ...
    async def _sha(self, ref: str) -> str:
        rc, out = await git_flow._git(str(self.repo), "rev-parse", ref)
        return out.strip()
    ...
    async def test_divergencia_no_toca_el_local(self) -> None:
        ...
        await git_flow.fetch_origin_safe(str(self.repo))
        before = await self._sha("develop")
        ref = await git_flow.sync_base_with_origin(str(self.repo), "develop")
...
class TestConversationDiff(unittest.IsolatedAsyncioTestCase):
    ...
    async def asyncSetUp(self) -> None:
        ...
        self.repo = Path(self._tmp.name)
        _init_repo(self.repo)
        self.branch = await git_flow.open_conversation_branch(
    ...
    async def test_separa_commiteado_de_pendiente(self) -> None:
        ...
        out = await git_flow.conversation_diff(str(self.repo), self.branch)
...
class TestCleanupBranch(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_no_borra_ramas_protegidas(self) -> None:
        ...
        self.assertTrue(await git_flow._branch_exists(str(self.repo), "main"))
...
class TestFinalizePr(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_body_builder_ve_el_diff_real(self) -> None:
        ...
        await git_flow.finalize_conversation_pr(
...
class TestConversationBranchHttp(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_nuevo_creates_branch_and_rejects_second(self) -> None:
        ...
        async with TestClient(TestServer(app)) as client:
            ...
            body = await r.json()
            self.assertEqual(body["branch"], git_flow.branch_name("usuario-demo-a"))
            self.assertEqual(body["base_branch"], "main")
            # el repo quedó parado en la rama nueva
            rc, out = await git_flow._git(
                str(self.repo), "rev-parse", "--abbrev-ref", "HEAD")
            self.assertEqual(out.strip(), body["branch"])

            # una segunda conversación abierta sobre el mismo repo → 409
            r2 = await client.post("/conversations", json={
                "project": "demo", "author": "otro"})
            self.assertEqual(r2.status, 409)
            self.assertEqual((await r2.json())["conversation_id"], body["id"])

    async def test_nuevo_sin_commit_local_arranca_sin_rama(self) -> None:
        """Repo `git init` sin commit local (HEAD unborn): la conversación
        arranca SIN rama en vez de 422. Caso real (code-hero-rpg): rewrite
        local no commiteado sobre un origin con otro proyecto — forzar el
        checkout de la base habría borrado el trabajo untracked."""
        from relay.server import create_app
        ...
        async with TestClient(TestServer(app)) as client:
            ...
            self.assertIsNone((await r.json())["branch"])
    ...
    async def test_diff_endpoint_devuelve_el_diff_de_la_rama(self) -> None:
        ...
        async with TestClient(TestServer(app)) as client:
            ...
            (self.repo / "hecho.txt").write_text("cambio\n", encoding="utf-8")
            _git(self.repo, "add", "-A")
            _git(self.repo, "commit", "-m", "trabajo")
            (self.repo / "hecho.txt").write_text("cambio\nsin commitear\n",
                                                 encoding="utf-8")
